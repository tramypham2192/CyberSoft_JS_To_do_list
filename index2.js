...
addButton.onclick = function(){
    ...
    if (taskArray.length != 0){
        ...
        checkTaskTrung(taskArray, newTaskValue);
        if (checkTaskTrung(taskArray, newTaskValue) == false && checkInputEmpty(newTaskValue) == false){
...
function renderTaskHtml(array){
    // localStorage.setItem("taskArrayToDoStorage", JSON.stringify(taskArray));
    // taskArrayToDoStorage = JSON.parse(localStorage.getItem("taskArrayToDoStorage"));
    let html ='';
    if (array == 0){
...
function xoaTask(value){
    console.log("task array inside xoaTask function: " + taskArray);    
    for (let j = 0; j < taskArray.length; j++){
        console.log("task array thu j: " + taskArray[j].taskName);
        let taskDelete = new Task();
        taskDelete.id = taskArray[j].id;
        taskDelete.taskName = taskArray[j].taskName; 
